...
def add_appointment(date, time, description, doctor):
    ...
    c.execute("INSERT INTO Appointments (Date, Time, Description, Doctor) VALUES (?, ?, ?, ?);", (date, time, description, doctor))
    conn.commit()
    conn.close()
...
# Streamlit UI for appointment fixing
def appointment():
    st.title("Appointment Fixing")
    ...
    description = st.text_input("Appointment Description")

    # Doctor selection
    doctors = ['Dr. Dhoni', 'Stokes', 'Dr. Jadeja']  # Example list of doctors
    doctor = st.selectbox("Select Doctor", doctors)

    # Fix Appointment button
    if st.button("Fix Appointment"):
        # if description.strip() == "":
        #     st.error("Please enter a description for the appointment.")
        # else:
        delete_appoints()
        add_appointment(date, time.strftime("%H:%M"), description, doctor)
        st.success("Appointment fixed successfully!")
    ...
    if appointments:
        for appointment in appointments:
            st.write(f"- Date: {appointment[0]}, Time: {appointment[1]}, Doctor: {appointment[3]}, Description: {appointment[2]}")
    else:
        st.write("No appointments found.")

# Streamlit UI for doctor menu
def doctor_menu():
    st.title("Doctor Menu")

    selected_doctor = st.selectbox("Select Doctor", ['Dr. Smith', 'Dr. Johnson', 'Dr. Lee'])

    if selected_doctor == 'Dr. Smith':
        st.write("Dr. Smith's Menu:")
        # Add menu items specific to Dr. Smith
    elif selected_doctor == 'Dr. Johnson':
        st.write("Dr. Johnson's Menu:")
        # Add menu items specific to Dr. Johnson
    elif selected_doctor == 'Dr. Lee':
        st.write("Dr. Lee's Menu:")
        # Add menu items specific to Dr. Lee

# Main function to run the Streamlit app
def mains():
    st.sidebar.title("Navigation")
    page = st.sidebar.radio("Go to", ["Appointment Fixing", "Doctor Menu"])

    if page == "Appointment Fixing":
        appointment()
    elif page == "Doctor Menu":
        doctor_menu()
...
def admin():
    ...
    ## CUSTOMERS
        ...
        st.subheader("By vishal (825), dheyshnan (795) & Surya (763)")
...
def customer(username, password):
    if getauthenicate(username, password):
        print("In Customer")
        st.title("Welcome to Health care logistics")
...
if _name_ == '_main_':
    ...
    menu = ["Login", "SignUp","Admin","Appointment fixing"]
    choice = st.sidebar.selectbox("Menu", menu)
    if choice == "Login":
        ...
        if username == 'admin' and password == 'admin':
            admin()
    elif choice == "Appointment fixing":
        username = st.sidebar.text_input("User Name")
        password = st.sidebar.text_input("Password", type='password')
        if username =="customer" and password == 'admin':
            mains()